...
class Predictor:
    ...
    def process(self, raw):
        ...
        def _batched_process(batched_img):
            ...
            layout = core.label_as_rgb_visual(outputs.data.cpu())
...
def image(path, weight, input_size):
    demo = Predictor(input_size, weight_path=weight)

    img = np.array(Image.open(path))
    output = demo.process(img[:, :, ::-1])
...
@click.option('--weight', type=click.Path(exists=True))
@click.option('--input_size', default=(320, 320), type=(int, int))
def video(device, path, weight, input_size):
    demo = Predictor(input_size, weight_path=weight)
    cap = cv2.VideoCapture(path if path else device)
