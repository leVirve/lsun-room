#Adding HHA features of SUNRGBD for depth estimation pretrain
#Still Based on ResNet101
...
# RGB branch
        self.s_classifier = nn.Conv2d(2048, num_classes, kernel_size=1, stride=1, bias=False)
...
# Depth branch
        self.d_dconv1 = nn.ConvTranspose2d(2048, 512, kernel_size=3, stride=1, bias=False)
        self.d_dconv2 = nn.ConvTranspose2d(512, 256, kernel_size=3, stride=1, bias=False)
        self.d_dconv3 = nn.ConvTranspose2d(256, 128, kernel_size=3, stride=1, bias=False)
        self.d_dconv4 = nn.ConvTranspose2d(128, 64, kernel_size=3, stride=1, bias=False)
        self.d_classifier = nn.ConvTranspose2d(64, 1, kernel_size=16, stride=8, bias=False)
...
#		p2 = self.avgpool(x4)
        ...
        x = self.dp2(x)
# RGB Branch
        rgb = self.s_classifier(x)
        upscore = self.upscore(rgb)
...
# HHA Branch
        hha = self.d_dconv1(x)
        hha = self.d_dconv2(hha)
        hha = self.d_dconv3(hha)
        hha = self.d_dconv4(hha)
        hha = self.d_classifier(hha)
        out_d = hha[:,:, 32:32+in_.size()[2], 32:32+in_size()[3]].contiguous()     
