...
class Stage_Net():
    ...
    def __init__(self, name, pretrained, stage_2, joint_class, type_portion, edge_portion):
        ...
        self.model = build_resnet101_FCN(nb_classes=37, stage_2=stage_2, joint_class=joint_class).cuda()
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-4)
        self.scheduler = ReduceLROnPlateau(self.optimizer, verbose=True, patience=2, mode='min', min_lr=1e-12)
        self.tf_summary = Logger('./logs', name=name)
        self.criterion = Joint_Loss(l1_portion=0.1, edge_portion=edge_portion, type_portion=type_portion)
    ...
    def train(self, train_loader, validate_loader, epochs):  		
        for epoch in range(1, epochs+1):
            self.model.train()
            self.epoch = epoch
            hist = EpochHistory(length=len(train_loader), joint_class=self.joint_class)
            ...
            for data in progress:
                ...
                if  not self.stage_2:
                    ...
                    loss, loss_term, acc = self.forward(image, target, edge_map, room_type)
                else:
    ...
    def evaluate(self, data_loader, prefix=''):
        self.model.eval()
        hist = EpochHistory(length=len(data_loader), joint_class=self.joint_class)
        for i, (data) in enumerate(data_loader):
            if not self.stage_2:
                    loss, loss_term, acc, output = self.forward(data[0], data[1], is_eval=True)
                    hist.add(loss, loss_term, acc)
            else:
